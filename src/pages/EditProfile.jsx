...
import {
    Layers, Palette, UserCircle, Contact, Images as ImagesIcon, Share2, CheckCircle, Plus, Trash2, Camera, UploadCloud, Link, X, Image as ImageIcon, Save, Smartphone, ChevronLeft, ChevronRight, UserPlus, Globe, MapPin, Building, Phone, Mail, Loader2
} from "lucide-react";
...
const EditProfile = () => {
    ...
    // State for raw file objects and existing image URLs
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                setIsLoading(true);
                const response = await fetch(`${API_BASE_URL}/api/profile/${profileId}`, { credentials: 'include' });
                if (!response.ok) {
                ...
                if (!profile) {
                    throw new Error("Profile data not found in the server response.");
                }
                ...
                setAddressLink(profile.addressLink || "");
                setSocials(profile.socials || []);

                // Flatten theme object
                if (profile.theme) {
                    setTemplate(profile.theme.template || "template2");
                    setShowGallery(profile.theme.showGallery !== false);
                    setShowSocials(profile.theme.showSocials !== false);
                    setPrimaryColor(profile.theme.primaryColor || "#007A8A");
                    setAccentColor(profile.theme.accentColor || "#00AEEF");
                    setIconColor(profile.theme.iconColor || "#00AEEF");
                    setTitleTextColor(profile.theme.titleTextColor || "#FFFFFF");
                    setBioTextColor(profile.theme.bioTextColor || "#E5E7EB");
                    setFontFamily(profile.theme.fontFamily || "'Inter', sans-serif");
                }

                // Handle images
                if (profile.profileImageUrl) {
                    const fullUrl = `${API_BASE_URL}${profile.profileImageUrl}`;
                    setProfileImageUrl(fullUrl);
                    setExistingProfileImageUrl(profile.profileImageUrl);
                }
                if (profile.galleryImages) {
                    setGalleryImages(profile.galleryImages.map(img => `${API_BASE_URL}${img}`));
                    setExistingGalleryImages(profile.galleryImages);
                }
    ...
    const handleGalleryFileSelect = (e) => {
        ...
        const remainingSlots = MAX_IMAGES - (existingGalleryImages.length + galleryImageFiles.length);
    ...
    const removeGalleryImage = (index) => {
        const imageUrlToRemove = galleryImages[index];

        // Check if it's a blob URL (newly added) or an existing image URL
        if (imageUrlToRemove.startsWith('blob:')) {
            const fileIndex = galleryImages.filter(img => img.startsWith('blob:')).findIndex(img => img === imageUrlToRemove);
            if (fileIndex !== -1) {
                setGalleryImageFiles(prev => prev.filter((_, i) => i !== fileIndex));
            }
        } else {
            const serverPath = imageUrlToRemove.replace(`${API_BASE_URL}`, '');
            setExistingGalleryImages(prev => prev.filter(img => img !== serverPath));
        }
    ...
    const handleUpdate = async () => {
        ...
        // Re-nest theme object
        const theme = {
            template, showGallery, showSocials, primaryColor, accentColor, iconColor,
            titleTextColor, bioTextColor, fontFamily
        };

        const dataToSubmit = {
            name, title, company, jobTitle, phones, emails, website, address, addressLink, socials, theme,
            existingProfileImageUrl: profileImageFile ? '' : existingProfileImageUrl, // Clear existing if new one is uploaded
        ...
        console.log("Submitting FormData to backend for update...");
        ...
        try {
            ...
            if (result.profile?._id) {
               navigate(`/profile-link/${result.profile._id}`, { state: { action: 'edited' } });
            } else {
                console.error("Profile updated, but no _id returned:", result);
            }
    ...
    // --- Unchanged Logic (with minor adjustments) ---
    ...
    // --- Side Effects (VCard generation, cleanup) ---
    useEffect(() => {
        ...
        return () => {
            ...
            // Cleanup blob URLs created for previews
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center h-screen bg-canvas-gray">
                <Loader2 className="h-12 w-12 animate-spin text-brand-teal" />
                <p className="ml-4 text-lg font-semibold">Loading Profile...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex items-center justify-center h-screen bg-canvas-gray text-destructive">
                <X className="h-12 w-12 mr-4" />
    ...
    const contactItems = [
        ...phones.filter(p => p.number).map(p => ({ id: p.id || p._id, href: `tel:${p.number}`, icon: PHONE_TYPES.find(t => t.label === p.type)?.icon || Phone, value: p.number, type: `${p.type} Phone` })),
        ...emails.filter(e => e.address).map(e => ({ id: e.id || e._id, href: `mailto:${e.address}`, icon: Mail, value: e.address, type: `${e.type} Email` })),
        ...
        <div className="min-h-screen bg-canvas-gray font-sans text-text-primary lg:h-screen lg:overflow-hidden">
            ...
            <Dialog open={isGalleryModalOpen} onOpenChange={setGalleryModalOpen}>
                <DialogContent>
                    ...
                    <Button asChild className={`w-full rounded-soft-pill ${galleryImages.length >= MAX_IMAGES ? 'opacity-50 pointer-events-none' : ''}`}>
                        <Label htmlFor="file-upload" className="cursor-pointer"><UploadCloud className="mr-2 h-4 w-4" /> Upload Images (Max {MAX_IMAGES})</Label>
                    </Button>
                    <p className="text-sm text-center text-muted-foreground">You have {galleryImages.length} of {MAX_IMAGES} images.</p>
            ...
            <div className="mx-auto w-full max-w-7xl p-4 md:p-8 lg:h-full">
                <div className="flex flex-col gap-8 lg:h-full lg:flex-row">
                    ...
                    <div className={`flex w-full flex-col rounded-xl bg-white shadow-lg lg:h-full lg:w-2/3 ${isMobilePreviewOpen ? 'hidden' : ''}`}>
                        <div className="p-6">
                            <h1 className="border-b border-canvas-gray pb-4 text-3xl font-extrabold text-brand-teal">Edit Profile</h1>
                        </div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center space-x-4">
                                                    {profileImageUrl ? 
                                                        ...
                                                        <div className="h-16 w-16 rounded-full border-2 border-canvas-gray bg-gray-200 flex items-center justify-center"><UserCircle className="h-8 w-8 text-gray-400" /></div>
                                                    }
                                        ...
                                        <div className="space-y-6">
                                            <div className="space-y-4">
                                                ...
                                                {phones.map((item, index) => (
                                                    <div key={item.id || item._id} className="rounded-lg border bg-canvas-gray/50 p-4">
                                                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-2">
                                                            <div className="space-y-1.5">
                                                                <Label htmlFor={`phone-number-${item.id || item._id}`} className={labelTextStyles}>Phone Number {index + 1}</Label>
                                                                <Input id={`phone-number-${item.id || item._id}`} type="tel" value={item.number} onChange={e => updateContact(setPhones, item.id || item._id, 'number', e.target.value)} className={formInputStyles} placeholder="e.g., +1 555 123 4567" />
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <Label htmlFor={`phone-type-${item.id || item._id}`} className={labelTextStyles}>Type</Label>
                                                                <div className="flex items-center space-x-2">
                                                                    <Select value={item.type} onValueChange={val => updateContact(setPhones, item.id || item._id, 'type', val)}>
                                                                        <SelectTrigger id={`phone-type-${item.id || item._id}`} className={`${formInputStyles} w-full`}><SelectValue /></SelectTrigger>
                                                                        <SelectContent>
                                                                            {PHONE_TYPES.map(t => <SelectItem key={t.label} value={t.label}>{t.label}</SelectItem>)}
                                                                        </SelectContent>
                                                                    </Select>
                                                                    <Button variant="ghost" size="icon" className="shrink-0 rounded-full text-gray-500 hover:bg-destructive/10 hover:text-destructive" onClick={() => removeContact(setPhones, item.id || item._id)}><Trash2 className="h-4 w-4" /></Button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                {emails.map((item, index) => (
                                                     <div key={item.id || item._id} className="rounded-lg border bg-canvas-gray/50 p-4">
                                                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-2">
                                                            <div className="space-y-1.5">
                                                                <Label htmlFor={`email-address-${item.id || item._id}`} className={labelTextStyles}>Email Address {index + 1}</Label>
                                                                <Input id={`email-address-${item.id || item._id}`} type="email" value={item.address} onChange={e => updateContact(setEmails, item.id || item._id, 'address', e.target.value)} className={formInputStyles} placeholder="email@example.com" />
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <Label htmlFor={`email-type-${item.id || item._id}`} className={labelTextStyles}>Type</Label>
                                                                <div className="flex items-center space-x-2">
                                                                    <Input id={`email-type-${item.id || item._id}`} value={item.type} onChange={e => updateContact(setEmails, item.id || item._id, 'type', e.target.value)} className={`${formInputStyles} w-full`} placeholder="e.g., Personal" />
                                                                    <Button variant="ghost" size="icon" className="shrink-0 rounded-full text-gray-500 hover:bg-destructive/10 hover:text-destructive" onClick={() => removeContact(setEmails, item.id || item._id)}><Trash2 className="h-4 w-4" /></Button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="space-y-4">
                                                {socials.map(item => {
                                                    ...
                                                    return (
                                                        <div key={item.id || item._id} className="space-y-4 rounded-lg border bg-background p-4 shadow-sm">
                                                            <div className="flex items-center justify-between border-b pb-3">
                                                                ...
                                                                <Button variant="ghost" size="icon" className="h-7 w-7 rounded-full text-gray-500 hover:text-destructive hover:bg-destructive/10" onClick={() => removeContact(setSocials, item.id || item._id)}><Trash2 className="h-4 w-4" /></Button>
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <Label className={labelTextStyles}>Handle / Display Text</Label>
                                                                <Input className={formInputStyles} value={item.handle} onChange={e => updateContact(setSocials, item.id || item._id, 'handle', e.target.value)} />
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <Label className={labelTextStyles}>Full Link (URL)</Label>
                                                                <Input className={formInputStyles} type="url" value={item.link} onChange={e => updateContact(setSocials, item.id || item._id, 'link', e.target.value)} />
                                                            </div>
                                                        </div>
                                                    );
                                                })}
                        ...
                        <div className="border-t border-canvas-gray p-6">
                            <Button onClick={handleUpdate} className="h-12 w-full rounded-soft-pill py-3 text-base font-bold"><Save className="mr-2 h-5 w-5" /> Update Profile Settings</Button>
                        </div>
                    </div>
                                ...
                                {showSocials && <div id="preview-socials-section">
                                    ...
                                    <div className="space-y-3 pb-16 text-left">
                                        {socials.filter(s => s.link).map(s => {
                                            ...
                                            return <SocialCard key={s.id || s._id} href={s.link} platform={platform} handle={s.handle} isImmersive={isImmersive} />;
                                        })}
                                    </div>
                                </div>}
                            </div>
...
const Template1Header = ({ profileImageUrl, name, title, firstPhone, firstEmail, addressLink }) => (
    <div className="p-6 pt-10 text-center text-white" style={{ backgroundColor: 'var(--primary-color)' }}>
        {profileImageUrl ? 
            ...
            <div className="mb-4 h-[250px] w-full rounded-xl border-4 border-white bg-gray-200 flex items-center justify-center"><ImageIcon className="h-16 w-16 text-gray-400" /></div>
        }
...
const Template2Header = ({ profileImageUrl, name, title, firstPhone, firstEmail, addressLink }) => (
    <div className="p-6 pt-10 text-center text-white" style={{ backgroundColor: 'var(--primary-color)' }}>
        {profileImageUrl ? 
            ...
            <div className="mb-4 h-32 mx-auto w-32 rounded-full border-4 border-white bg-gray-200 flex items-center justify-center"><UserCircle className="h-16 w-16 text-gray-400" /></div>
        }
        <h1 className="mb-1 text-3xl font-bold" style={{ color: 'var(--title-colour)' }}>{name}</h1>
        <p className="mb-6 text-base font-medium" style={{ color: 'var(--bio-colour)' }}>{title}</p>
        <div className="mb-4 flex justify-center space-x-4">
